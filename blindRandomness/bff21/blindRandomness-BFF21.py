...
P_WIN_CBOUND = 0.75           # Winning probability classical bound
...
ZERO_CLASS = ['1','2a'] # ['1','2a','2b','2b_swap','2c','3a','3b']
ZERO_TOLs = [1e-9] #[1e-9, 1e-5, 1e-3]                 # Tolerate error for zero classes
...
def objective(A, B, inputs, Z_ab, t_i):
    ...
    for a in range(2):
        for b in range(2):
            ...
            obj += t_i * meas_B[b]*(Z_ab[ab]*Dagger(Z_ab[ab]))
    return obj
...
def innerQuad(level, P, Z_ab, inputs, quad_t, quad_w, inp_probs = [], p_win_constr = [],
              zero_constr = [], zero_class = '', substs = {}, extra_monos = []):
    A, B = P.parties
    ...
    p_win_in_quad = sdp[winProb(P, inp_probs=inp_probs)]
...
def singleRoundEntropy(P, Z_ab, win_prob, M, inputs, inp_probs = [], zero_class = '', zero_tol = 0,
                        substs = {}, extra_monos = [], win_tol = WIN_TOL, level = LEVEL):    

    p_win_constr = [winProb(P, inp_probs=inp_probs)-win_prob+win_tol,
                    -winProb(P, inp_probs=inp_probs)+win_prob+win_tol]
    # p_win_constr = [winProb(P)-win_prob+win_tol]
    zero_pos = KS_ZEROS.get(zero_class, [])
    ...
    results = Parallel(n_jobs=N_WORKER_QUAD, verbose=0)(
                                delayed(innerQuad)(level, P, Z_ab, inputs, T[i], W[i], inp_probs,
    ...
    for inputs in INPUTS:
        ...
        #         print(f'Chosen input xy={inputs[0]}{inputs[1]}')         
            ...
            results = Parallel(n_jobs=N_WORKER_LOOP, verbose = 0)(
                                delayed(singleRoundEntropy)(P, Z_ab, win_prob, M,
                                                            inputs, inp_probs,
                                                            zero_class, zero_tol,
                                                            substs, extra_monos) \
